...
class GitShowInformation
{
    // ===== PUBLIC =====
    public function loadGitInformation(Project $project, ?string $gitPath): array
    {
        if (blank($gitPath) || ! is_dir($gitPath)) {
            return [
                ...
                'repository_path' => '-',
                'repository_size' => $gitPath
                    ? $this->getRepositorySize($gitPath)
                    : '-',
                'git_version' => '-',
                'branch' => '-',
                'default_branch' => '-',
    ...
    public function getRepositoryStatus(?string $gitPath): array
    {
        if (
            blank($gitPath) ||
            ! is_dir($gitPath) ||
    ...
    public function loadLatestCommits(?string $gitPath): array
    {
    ...
    public function loadBranches(?string $gitPath): array
    {
         if (! $this->validateRepository($gitPath)) {
            return [
                'local' => [],
    ...
    public function loadContributors(?string $gitPath): array
    {
        if (! $this->validateRepository($gitPath)) {
            return [];
        }
    ...
    public function loadWorkingTree(?string $gitPath): array
    {
        if (! $this->validateRepository($gitPath)) {
            return [
                'clean' => false,
    ...
    public function loadRepositoryHealth(?string $gitPath,array $repositoryStatus): array {
        if (!$this->validateRepository($gitPath)) {
            return [
                'score' => 0,
   ...
   private function directorySize(string $directory): int
    ...
    public  function validateRepository(?string $gitPath): bool
    {
